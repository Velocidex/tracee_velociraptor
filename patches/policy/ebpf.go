...
import (
	...
	"github.com/Velocidex/tracee_velociraptor/userspace/filters"
	"github.com/Velocidex/tracee_velociraptor/userspace/logger"
	"github.com/Velocidex/tracee_velociraptor/userspace/proc"
)
...
func createNewInnerMapEventId(m *bpf.Module, mapName string, mapVersion uint16, eventId events.ID) (*bpf.BPFMapLow, string, error) {
	spec, pres := m.Specs.Maps[mapName]
	if !pres {
		return nil, "", fmt.Errorf("Unknown spec for map %v", mapName)
	}

	copy_spec := spec.Copy()
	copy_spec.Name = fmt.Sprintf("%s_%d_%d", mapName, mapVersion, uint32(eventId))

	res, err := ebpf.NewMap(copy_spec)
	if err != nil {
		return nil, "", err
	}

	return &bpf.BPFMapLow{res}, copy_spec.Name, nil
}
...
func createNewInnerMap(m *bpf.Module, mapName string, mapVersion uint16) (*bpf.BPFMapLow, error) {
	...
	copy_spec := spec.Copy()
	copy_spec.Name = fmt.Sprintf("%s_%d", mapName, mapVersion) // new map name
...
func SetPolicy(ps *policies, id int, p *Policy) error {
	return set(ps, id, p)
}
