...
import (
	...
	"github.com/Velocidex/tracee_velociraptor/userspace/bitwise"
...
func NewPolicies() *policies {
	return &policies{
		...
		uidFilterMin:            filters.GetUnsetMin[uint64](),
		uidFilterMax:            filters.GetUnsetMax[uint64](),
		pidFilterMin:            filters.GetUnsetMin[uint64](),
		pidFilterMax:            filters.GetUnsetMax[uint64](),
...
var _ interfaces.Cloner[*policies] = &policies{}
...
func (ps *policies) matchedNames(matched uint64) []string {
	...
	for _, p := range ps.allFromMap() {
		if bitwise.HasBit(matched, uint(p.ID)) {
